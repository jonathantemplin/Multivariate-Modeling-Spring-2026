...
needed_packages = c("lavaan","semPlot", "fields", "mnormt")
...
summary(model00.fit, fit.measures=TRUE, standardized=TRUE)
...
summary(model05.fit, fit.measures=TRUE, standardized=TRUE, rsquare=TRUE)
...
#model estimation
model08.fit = sem(model08.syntax, data=math_data, estimator = "MLR", mimic="Mplus", fixed.x=FALSE)

#display model output
summary(model08.fit, fit.measures=TRUE, standardized=TRUE)




#plot path diagram with standardized coefficients
semPaths(model08.fit,intercepts = FALSE, residuals = TRUE, style="mx", layout="spring", rotation=1,
